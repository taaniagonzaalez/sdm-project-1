...
# Puedes cambiar esto por el tema que quieras
DEFAULT_QUERY = "graph databases"

# Cantidad de papers semilla que vas a buscar
SEARCH_LIMIT = 10
...
class GraphBuilder:
    ...
    def infer_venue_type(self, paper: dict, dblp_info: Optional[dict]) -> str:
        # Heurística simple:
        # - Si journal está presente -> journal
        # - Si publicationTypes contiene JournalArticle -> journal
        # - Si venue/title parece workshop -> workshop
        # - En otro caso -> conference
    ...
    def build_publication_container(self, paper: dict, dblp_info: Optional[dict]) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str]]:
        ...
        if venue_type == "workshop":
            if container_id not in self.workshops:
                self.workshops[container_id] = {
                    "workshop_id": container_id,
                    "name": container_name
                }
        else:
            if container_id not in self.conferences:
                self.conferences[container_id] = {
                    "conference_id": container_id,
                    "name": container_name
                }
        ...
        # City casi nunca viene desde estas APIs para cada edición.
        # Añadimos una ciudad sintética mínima y explícita.
    ...
    def export_all(self, output_dir: Path):
        ...
        # Nodes
        self.write_csv(output_dir / "papers.csv",
                       ["paper_id", "title", "year", "venue", "doi", "arxiv", "citation_count", "reference_count"],
                       list(self.papers.values()))

        self.write_csv(output_dir / "authors.csv",
                       ["author_id", "name"],
                       list(self.authors.values()))

        self.write_csv(output_dir / "topics.csv",
                       ["topic_id", "name"],
                       list(self.topics.values()))

        self.write_csv(output_dir / "abstracts.csv",
                       ["abstract_id", "text"],
                       list(self.abstracts.values()))

        self.write_csv(output_dir / "years.csv",
                       ["year_id", "value"],
                       list(self.years.values()))

        self.write_csv(output_dir / "cities.csv",
                       ["city_id", "name"],
                       list(self.cities.values()))

        self.write_csv(output_dir / "editions.csv",
                       ["edition_id", "name", "year", "event_name", "date"],
                       list(self.editions.values()))

        self.write_csv(output_dir / "proceedings.csv",
                       ["proceedings_id", "name"],
                       list(self.proceedings.values()))

        self.write_csv(output_dir / "volumes.csv",
                       ["volume_id", "name", "journal_name", "year"],
                       list(self.volumes.values()))

        self.write_csv(output_dir / "journals.csv",
                       ["journal_id", "name"],
                       list(self.journals.values()))

        self.write_csv(output_dir / "conferences.csv",
                       ["conference_id", "name"],
                       list(self.conferences.values()))

        self.write_csv(output_dir / "workshops.csv",
                       ["workshop_id", "name"],
                       list(self.workshops.values()))

        # Relationships
        self.write_csv(output_dir / "paper_cites_paper.csv",
                       ["src_paper_id", "dst_paper_id"],
                       [{"src_paper_id": s, "dst_paper_id": d} for s, d in sorted(self.rel_cites)])

        self.write_csv(output_dir / "paper_contains_topic.csv",
                       ["paper_id", "topic_id"],
                       [{"paper_id": p, "topic_id": t} for p, t in sorted(self.rel_contains)])

        self.write_csv(output_dir / "paper_starts_with_abstract.csv",
                       ["paper_id", "abstract_id"],
                       [{"paper_id": p, "abstract_id": a} for p, a in sorted(self.rel_starts_with)])

        self.write_csv(output_dir / "paper_published_at_edition.csv",
                       ["paper_id", "edition_id"],
                       [{"paper_id": p, "edition_id": e} for p, e in sorted(self.rel_published_at_edition)])

        self.write_csv(output_dir / "paper_published_at_volume.csv",
                       ["paper_id", "volume_id"],
                       [{"paper_id": p, "volume_id": v} for p, v in sorted(self.rel_published_at_volume)])

        self.write_csv(output_dir / "paper_appear_in_proceedings.csv",
                       ["paper_id", "proceedings_id"],
                       [{"paper_id": p, "proceedings_id": pr} for p, pr in sorted(self.rel_appear_in)])

        self.write_csv(output_dir / "edition_part_of.csv",
                       ["edition_id", "parent_label", "parent_id"],
                       [{"edition_id": e, "parent_label": label, "parent_id": pid}
                        for e, label, pid in sorted(self.rel_edition_part_of)])

        self.write_csv(output_dir / "edition_dated_in_year.csv",
                       ["edition_id", "year_id"],
                       [{"edition_id": e, "year_id": y} for e, y in sorted(self.rel_edition_dated_in)])

        self.write_csv(output_dir / "edition_placed_at_city.csv",
                       ["edition_id", "city_id"],
                       [{"edition_id": e, "city_id": c} for e, c in sorted(self.rel_edition_placed_at)])

        self.write_csv(output_dir / "edition_has_proceedings.csv",
                       ["edition_id", "proceedings_id"],
                       [{"edition_id": e, "proceedings_id": p}
                        for e, p in sorted(self.rel_edition_has_proceedings)])

        self.write_csv(output_dir / "volume_dated_in_year.csv",
                       ["volume_id", "year_id"],
                       [{"volume_id": v, "year_id": y} for v, y in sorted(self.rel_volume_dated_in)])

        self.write_csv(output_dir / "volume_part_of_journal.csv",
                       ["volume_id", "journal_id"],
                       [{"volume_id": v, "journal_id": j} for v, j in sorted(self.rel_volume_part_of)])

        self.write_csv(output_dir / "author_writes_paper.csv",
                       ["author_id", "paper_id", "role", "is_corresponding", "author_order"],
                       [{
                           "author_id": a,
                           "paper_id": p,
                           "role": role,
                           ...
                           "author_order": order
                       } for a, p, role, corr, order in sorted(self.rel_writes)])
